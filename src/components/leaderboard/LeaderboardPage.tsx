...
import { adaptLeaderboardEntry } from "@/lib/interfaces";
...
export function LeaderboardPage() {
  ...
  const entries = (Array.isArray(apiEntries) ? apiEntries : []).map(adaptLeaderboardEntry);
  const myUserId = apiUser?._id;
  ...
  return (
    <div className="p-5 max-w-4xl mx-auto space-y-5">
      ...
      {/* My Rank Banner */}
      {myEntry && (
        <div className="rounded-2xl p-4 text-white flex items-center justify-between" style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-slate-400 text-xs">
                {myEntry.averageScore}% avg · {myEntry.testsAttempted} tests · {myEntry.category} category
              </div>
            </div>
          </div>
        </div>
          ...
          {[top3[1], top3[0], top3[2]].map((entry, i) => {
            ...
            return (
              <div key={entry.rank} className={cn("flex flex-col items-center rounded-2xl border-2 p-4", colors[i], borders[i])}>
                ...
                <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-orange-700 font-bold text-sm mb-1">
                  {entry.name[0]}
                </div>
            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  isMe ? "bg-orange-100 text-orange-700" : "bg-gray-50 text-gray-500"
                )}>
                  {entry.rank <= 3 ? BADGE_ICONS[entry.badge] : `#${entry.rank}`}
                </div>

                <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style={{ background: "linear-gradient(135deg, #f97316 0%, #ea580c 100%)" }}>
                  {entry.name[0]}
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={cn("text-sm font-semibold truncate", isMe ? "text-orange-600" : "text-gray-800")}>
                      {entry.name}
                    </span>
                    {isMe && <span className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full font-medium">You</span>}
                  </div>
